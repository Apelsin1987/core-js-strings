...
function countVowels(str) {
  ...
  let sum = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (strVowels.includes(str[i])) sum += 1;
  }
  return sum;
}
...
function isPalindrome(str) {
  const newStrLC = str.toLowerCase();
  const newStr = newStrLC.replaceAll(/[\s.,#!?$%&;:{}=\-_`~()]/g, '');
  const mediumLength = Math.ceil(newStr.length / 2);
  const endSubstr = newStr.slice(-mediumLength).split('').reverse().join('');
  const startSubstr = newStr.slice(0, mediumLength);
